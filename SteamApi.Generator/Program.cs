...
var managerCode = Generator.GenerateManagerStubRegistration(api.Interfaces);
...
foreach (var shimExport in activeShims.OrderBy(x => x.Ordinal))
    if (shimExport.Ordinal > 0)
        exportsBodySb.AppendLine($"    {shimExport.Name} @{shimExport.Ordinal}");
    else
        // Fallback for items without specific ordinal (shouldn't happen for DLL match parity)
        exportsBodySb.AppendLine($"    {shimExport.Name}");
...
public static class Generator
{
    ...
    public static string GenerateLogicStub(InterfaceDef iface)
    {
        ...
        sb.AppendLine("    public object? UserContext { get; set => field = value; }");
        sb.AppendLine();

        foreach (var method in iface.Methods)
        {
            sb.Append($"    public {TypeDb.MapType(method.ReturnType)} {method.LogicName}({Generics(method.Params)})");
            sb.AppendLine();
            sb.AppendLine("    {");
            sb.AppendLine(
        ...
        List<InterfaceDef> interfaces, List<ExportInfo> exports, List<FunctionDef> functions)
    {
        ...
        // Build Version Map for SteamInternal_CreateInterface
        // Map "SteamUser023" -> "SteamUser_Entry"
        ...
        sb.AppendLine("    private static readonly ConcurrentDictionary<IntPtr, Action<IntPtr>> _activeShims = new();");
        sb.AppendLine(
        ...
        foreach (var acc in iface.Accessors)
        {
            ...
            sb.AppendLine($"        var logic = SteamEmulator.Manager.GetImplementation<{iface.ClassName}Logic>();");
        ...
        foreach (var func in functions)
        {
            ...
            activeShims.Add((funcName, ordinal));
            usedExportNames.Add(funcName);
    ...
    public static string GenerateManagerStubRegistration(List<InterfaceDef> interfaces)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated/>");
        sb.AppendLine("#nullable enable");
...
public static class TypeDb
{
    ...
    public static string MapType(string type)
    {
        ...
        var depth = 0;
        var resolved = type;
        while (_typedefs.TryGetValue(resolved, out var nextType))
        {
        ...
        if (_structs.Contains(type)) return "IntPtr";
